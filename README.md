...
**Brand color:** `e.html` exposes the header/subbar/icon/input-focus/checkbox colors as a `--brand-color` CSS custom property (default `#0C447C`, matching the original design). `event.js` (and `dashboard.js`/`profile.js` for their own headers) set it through `applyBrandColor()` in `src/brandColor.js`, which also sets the lighter `--brand-color-light` used by the subbar/icon/focus ring. Every brand surface carries white text, so `applyBrandColor()` enforces WCAG AA contrast (4.5:1) against white: a color that fails is darkened in HSL lightness only (same hue and saturation, so e.g. sky blue `#87CEEB` becomes a clear `#1B7CA4` rather than a muddy grey), and the lighter variant never goes below that contrast either. Registration and edit profile offer 8 preset swatches (all pass unchanged) plus a free `<input type="color">` picker; picking runs the same check, saves the readable color, shows a live header preview, and tells the vendor when their color was darkened. Because the check also runs at display time, colors saved before it existed are corrected on screen too.
...
**Login MFA:** every vendor login requires a second factor — Cognito's native email MFA (`auth/resource.ts`: `multifactor: { mode: 'REQUIRED', email: true }`). After a correct password, Cognito emails a 6-digit code to the vendor's address automatically (no enrollment step); the dashboard's login form then prompts for it (`#otp-card`) and calls `confirmSignIn` to complete sign-in. This applies to the very first login after registration too. The code is sent via SES using the same verified `SES_SENDER_EMAIL` identity as the confirmation emails (see [Submission confirmation email](#submission-confirmation-email)) — this account has SES production access, so any vendor's email address works, not just pre-verified ones.
...
- **Brand color theming is computed in JS** (`src/brandColor.js`); the CSS `color-mix()` in each page's `:root` is only the first-paint default before a vendor's color is applied.
