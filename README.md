...
**Login MFA (currently disabled):** the login form and `dashboard.js` are already wired to handle a Cognito email-MFA challenge (`#otp-card`, `confirmSignIn`) — after a correct password, Cognito would email a 6-digit code to the vendor's address and prompt for it before completing sign-in. It's not turned on yet because Cognito's email MFA requires SES-backed sending, which in turn requires this AWS account to have SES **production access** — without it, any vendor whose email isn't individually pre-verified in SES would never receive their code and get locked out. Once production access is granted, re-enable it in `auth/resource.ts` (`multifactor: { mode: 'REQUIRED', email: true }` plus `senders: { email: { fromEmail: process.env.SES_SENDER_EMAIL } }`).
